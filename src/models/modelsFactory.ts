      ...
      for (const item of items) {
        const promise = new Promise(async (resolve, reject) => {
          ...
          if (!!foundItem) {
            // do an update
            if (dbLogging) {
              logger.debug(`${modelName} Upsert with update for ${priKey}=${priVal} ${JSON.stringify(foundItem)}`);
            }
            ...
            try {
              ...
              return reject(
                `${modelName} Upsert with Update failed ${JSON.stringify(err.stack || err)} item=${JSON.stringify(
                  item,
                )}`,
              );
            }
            ...
            // do a create
            if (dbLogging) {
              logger.debug(`${modelName} Upsert with Create for ${priKey}=${priVal}`);
            }
            ...
            try {
              ...
              return reject(
                `${modelName} Upsert with Create failed ${JSON.stringify(err.stack || err)} item=${JSON.stringify(
                  item,
                )}`,
              );
            }
